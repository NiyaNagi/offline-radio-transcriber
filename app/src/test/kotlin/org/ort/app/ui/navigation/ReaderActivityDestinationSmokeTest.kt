...
// (`R_432_activation_thread_route`) — one real `ActivityScenarioRule` per `ReaderDestination` plus
...
class ReaderActivityDestinationSmokeTest {
    ...
    // onOpenThread)`), now has a real destination — `ActivationThreadRouting.resolveActivationThread`
    ...
    // real `threadId` ("overs-tonight-th") — exactly the "a thread exists" case this test proves;
    ...
    // semantics property survives to match on — confirmed by reading `Controls.kt` before writing
    ...
    fun `R_432_activation_thread_route`() {
        ...
        runReaderActivity(ReaderDestination.FREQUENCIES, sessionId = tonightSessionId) { rule ->
            ...
            // with the true origin as its back label — `Frequencies`, not the frequency drill-in
            // this was reached through (`NavHostCallbacks.onOpenActivationThread` closes that drill-in
            // first, the same reasoning its own doc comment gives) — and its own overs table is the
            ...
            // System back from T02 goes straight to `Frequencies` — the frequency drill-in it was
    ...
    // already occupies that space (`FailureHost.kt`'s own `HEADER_HEIGHT` kdoc) — true for every
    ...
    // unreachable — `results/ui-audit/rig-lost/CF06-settings-rig.png`, the coordinator's own
    ...
    // `FailureSignalsPolling`/`FailureMapper` — no `Scenarios.load`/`DebugFailureOverride` scenario
