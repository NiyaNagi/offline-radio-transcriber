...
public object SettingsPolling {
    ...
    public fun modeScreen(
        ...
        val rigLink = when (val state = RigStatus.state) {
            RigStatus.State.Absent ->
                SettingsModeSetRowViewState("Rig link", SettingsRigFacts.configuredRigLinkFallback(context))
            is RigStatus.State.Connected -> SettingsModeSetRowViewState(
                "Rig link",
                SettingsRigFacts.rigLinkSubLine(state.descriptor, state.transportKind, context, "connected"),
            )
            is RigStatus.State.Stale -> SettingsModeSetRowViewState(
                "Rig link",
                SettingsRigFacts.rigLinkSubLine(
    ...
    /** CF06's own entry point — the real logic (and every doc comment/requirement citation it
     * carries) lives in [SettingsRigFacts.rig] now, split out purely to keep this object under
     * detekt's `LargeClass` threshold; every existing caller/test keeps calling this one unchanged. */
    public fun rig(context: Context): SettingsRigViewState = SettingsRigFacts.rig(context)

    /** Unchanged delegate — `ui/setup/RadioVerifiedScreen.kt` (outside this package's ownership)
     * calls this exact name; see [SettingsRigFacts.stripManufacturerPrefix]'s own doc comment for
     * the rule itself, now split out with the rest of the CF06 block. */
    internal fun stripManufacturerPrefix(displayName: String): String =
        SettingsRigFacts.stripManufacturerPrefix(displayName)
