...
| `results/backlog.md` | **The single prioritised view** of everything still open — generated by `python tools/backlog/backlog.py` from the register, the checklists, the build plan, the questions and the coverage matrix. Never edit it; file the item on the surface that owns it and regenerate. `--check` fails when it is stale |
