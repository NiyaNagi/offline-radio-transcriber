...
public class AndroidAudioIoInterruptionGapTest {
    ...
    fun `FR_RUN_11 a real device-removed callback opens a gap of the true outage length and capture resumes`() =
        runTest {
            ...
            val descriptor =
                AudioDeviceDescriptor(info.id.toString(), AudioDeviceKind.USB_DEVICE, "USB Audio Adapter")
            ...
            val job = this.launch {
                source.start().collect { ev ->
                    ...
                    // -- firing it before that would find no listener registered yet, exactly the
                    // race a bare `launch { ... }` followed immediately by `removeInputDevice` would
                    // hit.
                    ...
                    if (ev is CaptureEvent.Interrupted) {
                        ...
                        clock.advance(7_000)
                        // The OS reports the adapter reconnected -- back in the enumeration before
                        // the backoff ladder's next retry reopens it.
            ...
            assertTrue(
                "a real callback-driven interruption must be reported",
