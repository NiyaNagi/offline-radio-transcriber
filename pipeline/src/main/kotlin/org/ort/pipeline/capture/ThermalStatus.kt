...
public object ThermalStatus {
    ...
    public sealed interface State {
        ...
        public data class Warm(override val osThermalStatus: Int, override val realTimeFactor: Double?) : State

        /** [THERMAL_STATUS_SEVERE] or worse. */
        public data class Hot(override val osThermalStatus: Int, override val realTimeFactor: Double?) : State
    }
    ...
    /** Republishes [state] from a fresh OS thermal status and whatever [recordPassTiming] has measured so far. */
    public fun sample(osThermalStatus: Int) {
        state = stateFor(osThermalStatus, smoothedRealTimeFactor)
    }

    /** Sets both facts at once — the scenario simulator's and tests' entry point. */
    public fun update(osThermalStatus: Int, realTimeFactor: Double?) {
        smoothedRealTimeFactor = realTimeFactor
        state = stateFor(osThermalStatus, realTimeFactor)
    }
    ...
    private fun stateFor(osThermalStatus: Int, realTimeFactor: Double?): State = when {
        osThermalStatus >= THERMAL_STATUS_SEVERE -> State.Hot(osThermalStatus, realTimeFactor)
        osThermalStatus >= THERMAL_STATUS_MODERATE -> State.Warm(osThermalStatus, realTimeFactor)
