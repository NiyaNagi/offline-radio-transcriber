...
| D15 | **Stack: Kotlin, Compose, Room/SQLite+FTS5, Coroutines/Flow, Hilt, WorkManager.** ASR and rig layers sit behind plain interfaces. **Amended 2026-09-19 (lead sweep): Hilt was never adopted.** The build takes no Hilt or Dagger dependency, it is absent from the version catalog, and no source file imports `dagger.hilt` or `javax.inject` — dependencies are passed by constructor from the composition root instead. This is recorded rather than quietly corrected because D15 is cited as the stack in `AGENTS.md` and the technical design, and an agent reading either would otherwise add a framework nothing uses. The substitution FR-TST-1..3 and AC-91 require is satisfied by constructor parameters plus the project's own behavioural fakes; nothing in the decision's rationale is lost. Found while assembling the third-party licence notices (NFR-6d), where listing Hilt would have been a false claim. | Product owner delegated the choice. Idiomatic, well-documented, and the interfaces are what keep sherpa-onnx and each radio replaceable |
...
| D51 | **Three tiers: Free, Pro, Teams.** This **amends D47** ("1.0 launches free, with no billing"). **Free** carries everything that makes the record trustworthy and everything the spec requires: capture in every mode, segmentation, Pass B, callsign resolution with its evidence priors, the four attribution states, the inspection ("why") surface, corrections, search, the digest, ADIF/CSV/POTA export (FR-EXP-1..5), live alerts (FR-ALR-1..6) and capture profiles (FR-CFG-1..3). **Pro** is a one-time purchase carrying convenience and depth that no requirement marks (M): the tiers are named in the entitlement requirements, not here. **Teams** carries the organisation surface — net roster and ICS-309/213 export — and is sold to organisations, **annually by invoice** by default, because county emergency-management offices and 990-filing search-and-rescue nonprofits purchase on annual purchase orders. **Nothing marked (M) in this specification may be gated behind any paid tier**, and neither may the attribution states, the inspection surface, corrections or basic export: a build that withholds a MUST is non-conforming. In-app purchase, if it ships, needs its own `:net` token kind and a `:billing` module under the same boundary rule as `:net` (Principle VII); an invoiced Teams licence needs neither. | Product owner direction, taken with the measured cost stated rather than sold: the revenue model puts a tiered consumer split at roughly **$1,200 over 36 months against $988 for a single paid listing** — about $200 of upside for 150–220 hours of work — because billing plus gated features cost more than a funnel of a few thousand installs returns, and the public Apache-2.0 source lets anyone rebuild without the gate. The organisation line is the only one with range (~$1,500/yr), which is why Teams exists and why it is sold the way its buyers actually purchase. Constitution 2.0.0 is what forces the "no MUST may be gated" rule: alerts and capture profiles became (M) requirements after the tiering research was written, so the obvious Pro contents are no longer gateable. See D47 (amended) |
| D52 | **Reversed: `:identity` (the voice library, D28) returns to 1.0, behind two gates.** This **amends D45**, which deferred FR-SPK-11..26 to post-1.0. Nothing about the honest labelling changes — a correction scoped by callsign is still called that. Two gates bind before any enrolment code merges: (1) the **R4 speaker-separation probe runs for real**, and because its corpus is out of domain for narrowband FM with squelch, a *pass* licenses nothing until a second arm runs on this project's own tape — only a *fail* is decisive, and a fail deletes most of D28 as a deliverable rather than a failure; (2) a **counsel memo** on BIPA, CUBI, Washington's My Health My Data and GDPR Art. 9 precedes any enrolment of a **third party** heard on air. Enrolment of the operator's own voice and of affirmatively consenting stations may be designed and built before the memo. Voiceprints stay **per device, never synced, never in any outbound channel or tier** (FR-SPK-20 unchanged), enrolment is geo-gated off by default where those statutes bite, and a voice match never yields `CONFIRMED` (FR-SPK-13). | Product owner direction. D45 was right that the voice library was never built and that claiming otherwise breached Principle I; it is reversed on scope, not on honesty. The gates exist because the biometric exposure is the highest-severity legal item in the roadmap and because the probe that would tell us whether separation works on this audio at all has still never been run — building the library before either answer arrives would be a guess wearing an implementation |
| D53 | **Reversed: the M4 fork is decided inside the 1.0 cycle.** This **amends D46**, which fixed the measurement to *after* 1.0. The metric is **dev-fold callsign precision at fixed recall, reported per source with fold, machine and execution provider** (constitution VI), measured for the text-level path and the Pass C prototype on the same tape and the same machine. The prototype is built only after the production Pass B honesty defects are fixed, because measuring Pass C against a Pass B that runs with no evidence priors measures the grammar, not the spotter. **An inconclusive result is recorded as the verdict**, not re-run until it reads well; a negative result deletes Pass C and its specification references, and that deletion is a deliverable. | Product owner direction: the fork has been open since the specification was written and carries debt in the spec, the tier ramp and the design canvas. Deciding it inside 1.0 closes that debt either way. The sequencing constraint is not negotiable — with the priors dead in production (`PassBFactory`), a head-to-head measured today would compare two broken arms |
...
| D51 Three tiers: Free, Pro, Teams; no (M) requirement may be gated | D47 (amended), FR-EXP-1..5, FR-ALR-1..6, FR-CFG-1..3, FR-UI-6, FR-UI-8 |
| D52 Voice library returns to 1.0 behind the R4 probe and a counsel memo | D45 (amended), D28, FR-SPK-11..26, FR-SPK-13, FR-SPK-20, R4 |
