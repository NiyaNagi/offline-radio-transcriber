...
| E2-A06 | FR-RIG-6, FR-CAP-5, FR-RIG-3 | **Owed data/descriptor additions found while wiring** (WPC2 report): TransmissionEntity.rigStateChangedMidTransmission (v9), CaptureGapCause.BLUETOOTH_AUDIO_LOST (v9), TransportSpec.usbVendorId/usbProductId/lineTerminator on :rig descriptors (then DefaultRigTransportFactory reads the descriptor before igParams), band-scoped requencyForTransmission(band) at the segment level (D23) | unit (`MigrationTest` v8→v9 + every fixture; `DefaultRigTransportFactoryTest` precedence; `RigSupervisorTest` D23 both-open → earlier band + `ambiguous`; `RealCaptureServiceBluetoothDropTest` cause — shown to discriminate; `ReconnectLadderTest`; `RigLinkBridge` tests ×5). VID/PID left absent on purpose — the reference marks them unverified; H1 fills them | WPC3 | closed — `7637d0a`, gate on main `8e40041` green |
| E2-A08 | constitution II (test integrity) | **Two gate-red flakes found while merging this program, both real defects:** (a) `FileHeartbeatStore.last()`/`hadUncleanEnd()` check `exists()` then read — a `FileNotFoundException` escapes into the UI when the file is deleted between the two (scenario reset, `clear()`, pm-clear); (b) `ReaderActivityDestinationSmokeTest.seedFrequencyOversPattern` anchors "tonight" to the wall clock while `buildNightlySequence` buckets by local calendar day, so for ~80 min after local midnight the "Busier than usual" action never renders and R_432/R_276 fail | unit — (a) `HeartbeatStoreTest` with a `File` whose `exists()` lies and a truncated line (AC-5/FR-PLT-1); (b) the seeding anchored to a fixed local time per calendar day + a boundary test at 00:05 local | WPC3 (a) / WPE (b) | closed - (a) `37ec05c7` (`HeartbeatStoreTest` ×3, `ExistsButGoneFile`), merged `e92f8565`, gate green; (b) fixed - WPE `32399e7e` (nights anchored to 21:00 local per calendar day; `R_276_R_432_seeding_pattern_is_immune_to_a_real_midnight_boundary` discriminates), merged `5872873d`; full gate green at 00:48 local (inside the old failure window) - closed |
| E2-A07 | FR-CAP-13, AC-129, DG04 | **Owed session facts found by the capture review** (R-825, R-833): the session row carries no rig descriptor id, no route-verified flag and no native sample rate, so DG04 cannot render "TH-D75A · Bluetooth SPP" or "USB Audio Device · verified · 48 kHz" for a past session without inventing them. v10 adds `rigDescriptorId`, `audioRouteVerified`, `audioNativeRateHz` on `SessionEntity`; `RealCaptureService` writes them at start from the configuration and the verifier's result; `SessionRouteFacts` renders them, falling back to the transport alone / no rate when null | unit (`MigrationTest` v9→v10 + fixtures; `RealCaptureServiceCaptureModeTest.AC_129_*` extended; `DigestPollingTest.FR_CAP_13_*` for the past-session rows) + tour (`bt-audio-session/DG04`, `mode-usb/DG04`) | WPC3 / WPF | fixed - WPC3 `46962b11` (v10 `MIGRATION_9_10`, `RealCaptureService` writes the id and native rate at start and the verified flag once it resolves, `SessionRouteFacts`/`DigestPolling` render them), merged `e92f8565`, gate running; tour half on run 3. Left open by design: `audioRouteVerified` stays null when a mid-session `RouteChanged` resolves before any `Frames` arrived |
...
| E2-B09 | FR-RIG-6 | Rig state correlated to transmissions by time; a mid-transmission change recorded at start and flagged | unit (`DescriptorRigModuleTest.FR_RIG_6_*`; the session-side correlation is WPC2) | WPA / WPC2 | closed (rig half) — `3bed6da`; session half closed — `e007969` + `7637d0a` (`RigSupervisorTest.FR_RIG_6`; flag persisted on `TransmissionEntity.rigStateChangedMidTransmission`, v9) |
...
| E2-D06 | FR-RIG-8, FR-RIG-9 | Frequency provenance `rig` while connected, `manual` on override, stale on drop; every frequency carries provenance | unit (`RigSupervisorTest` frequencyForTransmission: manual-first, rig with stale confidence, UNKNOWN) — **band-scoped attribution (D23) not yet wired at the segment level**: `RealCaptureService` passes `band = null`; owed | WPC2 / WPC3 | closed — `e007969` + `7637d0a` (band-scoped: one open → that band, neither → NONE, both → earlier-opened + `ambiguous`), gate `8e40041` green |
| E2-D07 | FR-CAP-5, F23 | A Bluetooth audio drop → `InputStatus.Lost` + a `CaptureGap`, retry ladder, recovery announced; never captures nothing silently | unit (`RealCaptureServiceBluetoothDropTest`) + device (`bt-audio-dropped`) + hardware H7 — cause now `BLUETOOTH_AUDIO_LOST` (`7637d0a`) | WPC2 / WPC3 | closed (unit) — `e007969`, `7637d0a` |
...
| E2-E04 | FR-CAP-2b, FR-CAP-10, AC-128 | S04 offers the built-in mic as a real choice, neutral tone, room-audio disclosure; `Verify` unlocks | unit (closed at `4fed9de`) + tour (`setup-mode/S04-input-mic`) | WPD | closed — enumerator `4fed9de`; captured in run 1 (`setup-mode/S04-input-mic`, reviewer A: matches the board apart from R-815's preset chip) |
| E2-E05 | FR-CAP-11, AC-132 | S04 lists the Bluetooth route with its narrowband disclosure; choosing it proceeds to verify | unit (closed at `4fed9de`) + tour | WPD | fixed — enumerator `4fed9de`; the Bluetooth-lane S04 capture rendered S02c in run 1 (R-820, AVD lacked `BLUETOOTH_CONNECT`) — closes on run 2 |
...
| E2-E10 | FR-RIG-14, S10b | S10b lists paired devices (SPP-capable selectable, headset-only not), `Pair in system settings`, the open→identify→verify checklist; `Continue` only after verify | unit (`RigBluetoothScreenTest` over `InMemoryRigLinkPort`) + tour (`setup-rig-bluetooth/S10b-*` per state) + device V8 — **real link blocked by design**: `:app` may not depend on `:rig-bluetooth` (ModuleGraph, VII); WPC3 adds a `:pipeline` `RigLinkBridge`, WPD then adapts `RigLinkPort` over it | WPD / WPC3 | fixed — `162dd62` (`BridgeRigLinkPort` over `DefaultRigLinkBridge`, `BridgeRigLinkPortTest` ×6 over `FakeRigLinkBridge`), gate green at `e47a2751`; the bridge now exposes only pipeline/rig/core types (`RigLinkSppSupport`, merged `37337a38`, gate green `20c3595d`); WPD's `BridgeRigLinkPort` is a plain `when` over it and `DebugRigLinkPortOverride` lets the tour script the paired list (`e2620507`, merged `b6a8b8a1`, gate green) — tour half closes on run 2; the live link itself is H2 |
...
| id | requirement | what must be true | verification | owner | status |
|---|---|---|---|---|---|
| E2-F01 | FR-CAP-12, CF02 | CF02 leads with the Capture-mode row; `Change` → CF11 | unit (`SettingsCaptureScreenTest`) + tour (`overnight/CF02-settings-capture`) | WPE | fixed (unit) — `a60d6e6`, gate green at `e47a2751`; tour: captured (`overnight/CF02`), but `mode-bluetooth/CF02` shows the default mode on an unseeded store (R-821) — re-judge on run 2 |
...
| E2-F08 | nav | Routes to CF11, S00 re-entry, S09b re-entry and F23's actions exist in `OrtNavHost`; `NavSeed` can seed CF11 | unit (`SettingsContentTest` MODE, `ReaderActivityDestinationSmokeTest.R_129_SETTINGS_MODE_*`, `FailureHostTest.FR_CAP_5_*` for the F23 callbacks firing, `ReaderActivityTest.FR_CAP_5_*` for the real `SetupActivity`/`EXTRA_STEP=INPUT` navigation) | WPE / WPF | closed — `a60d6e6`; F23's `onRetryInput`/`onSwitchToWiredInput` wired by WPF (pending), both routing to `ReaderNavigator.openSetupInput()` |

## G — Status, Now, Session, Log, Digest, failures (`app/ui/*`)

| id | requirement | what must be true | verification | owner | status |
|---|---|---|---|---|---|
| E2-G01 | FR-CAP-13, N04 | N04 Input sub-line names mode and route; Radio sub-line names the transport | unit (`CaptureStatusMapperTest.FR_CAP_13_*`/`FR_CAP_10_*`/`FR_CAP_11_*`/`FR_RIG_14_*` ×6, `CaptureStatusContentTest.FR_CAP_13_*` — renamed off the orphan `FR_CAP_13_*` prefix this session) + tour (`overnight-live/N04-capture-status`, `bt-audio-session/N04`) | WPF | closed (unit) — `bb952c6`, gate on main `22448af` green; tour open |
| E2-G02 | FR-CAP-3a, FR-CAP-10, AC-129, N01b | In local-mic mode the room-audio chip and the live-bar `room` mark appear on **every** destination that carries the live bar | unit (`LiveBarTest`/`LiveBarPollingTest`/`NowScreenTest`/`NowContentTest.FR_CAP_3a_*`/`FR_CAP_10_*`/`FR_CAP_12_*` — one shared `LiveBarPolling.current` feeds every destination; renamed off the orphan `FR_CAP_3a_*` prefix this session) + tour (`mode-local-mic/N01b-now`, `L01`, `T01`, `Q01`, `ST01`, `CF01` — one capture per destination) | WPF | closed (unit) — `bb952c6`, gate `22448af`; tour open |
| E2-G03 | FR-CAP-13, DG04 | DG04 shows Mode, Input (with profile for Bluetooth) and Rig link fact rows from the session row; R-450's deviation retired | unit (`DigestPollingTest.FR_CAP_13_*`/`FR_CAP_10_*`/`FR_CAP_11_*` ×4, `SessionsScreensTest.FR_CAP_13_*` — renamed off the orphan `FR_CAP_13_*` prefix this session) + tour (`overnight/DG04-session`, `bt-audio-session/DG04`, `mode-local-mic/DG04`) | WPF | closed (unit) — `bb952c6`, gate `22448af`; tour open |
| E2-G04 | FR-CAP-13, F23 | Every over from a Bluetooth-audio session carries the `bt audio` mark in L01, D01–D04 headers | unit (`LogViewDataTest`/`LogPollingTest`/`RowsTest`/`LogScreenTest.FR_CAP_13_*` for L01; `DetailViewStateMapperTest.FR_CAP_13_*`, `TransmissionDetailScreenTest.FR_CAP_13_*`, `TransmissionDetailContentTest.FR_CAP_13_*` for D01–D04, one case per attribution state) + tour (`bt-audio-session/L01-log`, D01–D04 open) | WPF | closed (unit, both halves) — `bb952c6`, gate `22448af`; D01–D04 detail-header mark built this session (pending), reading `SessionRouteFacts` through the transmission's own session; tour open |
| E2-G05 | FR-CAP-5, F23 | F23 banner from `InputStatus.Lost` on a Bluetooth route: gap counting, retry ladder, `Retry now`, `Switch to a wired input`; live bar reads `Input lost` | unit (`FailureMapperTest.FR_CAP_5_*`, `FailureScreensTest.FR_CAP_5_*` for the rendered "Retry N of M, next in S s." sentence, `FailureHostTest.FR_CAP_5_*`, `LiveBarPollingTest.FR_CAP_5_*` — mapped from the live `InputStatus.Lost.lastKnown` route and, since WPC3 (`7637d0a`), its own real `attempt`/`ofTotal`/`nextRetryInMillis`) + tour (`bt-audio-dropped/F23-now`, `-log`) + device V10 | WPF | closed (unit, both halves) — `bb952c6`, gate `22448af`; retry-ladder counter rendering built this session (pending) from WPC3's real fields, honest "Retrying automatically." fallback kept for a caller with none; tour/device open |
| E2-G06 | FR-RIG-15, F9 | F9 names the Bluetooth transport when that is what dropped; `rig-bt-lost` → `rig-reconnected` fires the recovery toast | unit (`FailureMapperTest.FR_RIG_15_*` — transport from the live `RigStatus.transportKind`; `FailureMapperTest`/`FailureScreensTest.FR_RIG_15_*` for the coordinator-directed retry-ladder addendum, WPC3's real `attempt`/`ofTotal`/`nextRetryInMillis` on `RigStatus.State.Stale`, rendered as the same "Retry N of M, next in S s." sentence F23 uses) + device V10 (the `-NoRestart` recipe) | WPF | closed (unit) — `bb952c6`, gate `22448af`; retry-ladder rendering built this session (pending) |
| E2-G07 | FR-DIG-3, FR-DIG-6, FR-DIG-11, DG05 | DG05 renders stored summaries badged `generated`, italic, attributed to overs, `Read the overs` → L01 filtered; absent entirely when disabled | unit (`DigestPollingTest.FR_DIG_6_*`/`FR_DIG_3a_*`/`FR_DIG_11_*` ×4 — absent-when-disabled shown to discriminate; `DigestScreensTest.FR_DIG_6_*`/`FR_DIG_3a_*`/`FR_DIG_11_*` ×3; `SessionsContentTest.FR_DIG_11_*` — renamed off the orphan `FR_DIG_6_*` prefix this session) + tour (`llm-enabled-prose/DG05-digest-prose`, `llm-disabled/DG01-digest`) | WPF / WPH | closed (unit) — `bb952c6`, gate `22448af`; tour open |
...
| id | requirement | what must be true | verification | owner | status |
|---|---|---|---|---|---|
| E2-J01 | plan § WPI | Every scenario named in the plan exists, seeds real stores/holders, and is documented in `results/ui-audit/README.md` | unit (`WpiScenariosTest` ×17) | WPI | closed — `711aecb`; gate on main after the WPI merge green (`e47a2751`, 240/450 covered) |
| E2-J02 | plan § WPI | `tour.json` has a step for every board in the plan's input list at 1.0 and 2.0, `-end` for long ones; `SetupStepIds` knows S00/S02c/S09b/S10b | tour run 1 on port 5558: **191 steps, 191 ok, 0 errors** (180 s), 40 new PNGs across 17 scenarios, `tour-manifest.json` committed | WPI | closed — `711aecb`, gate green at `e47a2751`; run 2 re-captures the steps reviewers found rendering S02c (R-810/R-820/R-830: AVD lacked `BLUETOOTH_CONNECT`) |
| E2-J03 | constitution VIII | Steps the tour cannot reach are named in the README with the reason, never faked | review — README names S10b's paired list (no `RigLinkPort` injection seam in `SetupActivity`; WPD adding `DebugRigLinkPortOverride`) | WPI | closed — `711aecb`, gate green at `e47a2751`; S10b fully reachable: paired list via `DebugRigLinkPortOverride`, four link states via `EXTRA_DEBUG_RIG_BLUETOOTH_ADDRESS` (WPI `a7f8641a`, merged `90d4d44b`); README "cannot reach" note deleted; confirm on run 3 |
