...
| E2-A08 | constitution II (test integrity) | **Two gate-red flakes found while merging this program, both real defects:** (a) `FileHeartbeatStore.last()`/`hadUncleanEnd()` check `exists()` then read — a `FileNotFoundException` escapes into the UI when the file is deleted between the two (scenario reset, `clear()`, pm-clear); (b) `ReaderActivityDestinationSmokeTest.seedFrequencyOversPattern` anchors "tonight" to the wall clock while `buildNightlySequence` buckets by local calendar day, so for ~80 min after local midnight the "Busier than usual" action never renders and R_432/R_276 fail | unit — (a) `HeartbeatStoreTest` with a `File` whose `exists()` lies and a truncated line (AC-5/FR-PLT-1); (b) the seeding anchored to a fixed local time per calendar day + a boundary test at 00:05 local | WPC3 (a) / WPE (b) | (a) fixed - `37ec05c7` (`HeartbeatStoreTest` ×3, `ExistsButGoneFile`), merged `e92f8565`; (b) fixed - WPE `32399e7e` (nights anchored to 21:00 local per calendar day; `R_276_R_432_seeding_pattern_is_immune_to_a_real_midnight_boundary` discriminates), merged `5872873d`, gate running |
