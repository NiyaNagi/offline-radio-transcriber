...
| E2-G05 | FR-CAP-5, F23 | F23 banner from `InputStatus.Lost` on a Bluetooth route: gap counting, retry ladder, `Retry now`, `Switch to a wired input`; live bar reads `Input lost` | unit (`FailureMapperTest.E2_G05_*`, `FailureHostTest.E2_G05_*`, `LiveBarPollingTest.E2_G05_*` — mapped from the live `InputStatus.Lost.lastKnown` route) + tour (`bt-audio-dropped/F23-now`, `-log`) + device V10 — backoff fields now published (`7637d0a`: `attempt`/`ofTotal`/`nextRetryInMillis` on `Lost` and `Stale`); rendering in WPF's follow-up | WPF | closed (unit) — `bb952c6`, gate `22448af`; counter rendering in flight |
