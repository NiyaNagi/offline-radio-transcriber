...
chooses the capture mode and presets the Input and Radio steps.
...
**Ten stages since 2026-09-20 (R-1087):** P22's MODELS and P28's ANALYTICS_CONSENT used to share
S12/Ready's own segment — both boards rendered an identical, full `8 of 8`, telling the operator
twice they were on the last step. Each now has its own position (`SetupStep.indicatorIndex()`),
and the counter reads `n of 10` — a fixed total (every stage this build's flow can ever reach, not
the subset a given run walks; see that function's own doc comment for why). The `counter now
'n of 8'` notes below are updated to `n of 10` in the same change; every position but MODELS'
(now 8) and ANALYTICS_CONSENT's (now 9, previously sharing 8 with Ready) is otherwise unchanged.

| # | Artboard | Purpose | Serves | Interactions | Status |
|---|---|---|---|---|---|
| S00 | `Setup-Mode.dc.html` | **How is the radio connected?** Three capture modes — local microphone, USB-connected radio, Bluetooth-connected radio — each stating what it presets (audio route, rig link) and what it costs. Stage 1 of 10 | **FR-CAP-8, FR-CAP-9**, D33 | Tap a mode → S02 (Bluetooth mode inserts S02c after S02). Footer states a change applies at the next session (FR-CAP-12) | drawn + built — tour runs 3–5 (`setup-mode/S00`, reviewers A2/A4), V8 and V11 on device (R-851 icon added). Accepted deviation: the row emphasis is on the recommended mode, not the chosen one (R-814/R-815) |
...
| S01a | `Setup-Jurisdiction.dc.html` | **First run only.** The legality of recording radio traffic varies by jurisdiction — states that plainly, once, right after Welcome (S01) and before Mode (S00); asks the operator to check their own local law. No step counter (like Welcome), no back chevron, no skip: capture cannot start until dismissed | **NFR-6c, AC-166** | `I understand` → S00 (Mode). Nothing else on the board | drawn — built already (P22, `JurisdictionNoticeScreen.kt`); this round adds the artboard and the S01a tour steps (`setup-mode` scenario base, the same fresh-store fixture S00 itself already uses), so it can now be captured and compared — not yet swept against a capture |
| S02 | `Setup-Mic.dc.html` | Microphone rationale before the OS prompt | FR-CAP-1 | `Allow microphone` → OS prompt → S03 (or S02c in Bluetooth mode). Denied → S02-denied | drawn + built — counter now `2 of 10` (P19 re-verifies; R-1087 renumbered the total) |
| S02b | `Setup-Mic-Denied.dc.html` | Microphone refused twice — the OS prompt is gone. Halting, with the settings path | FR-CAP-1, F1 | `Open app settings` → OS. `Check again` re-reads the grant; auto-advances to S03 when granted | drawn + built — counter now `2 of 10` |
...
| S03 | `Setup-Notify.dc.html` | Notification rationale — the persistent capture notification | FR-SVC-1, FR-PLT-1 | `Allow notifications` → S04. `Skip` → S04, with a warning noted | drawn + built — counter now `3 of 10` |
...
| S05 | `Setup-Verify.dc.html` | Verified-route check in progress, then pass | FR-CAP-2a, F1 | Auto-advances on pass → S07. `Retry` on fail | drawn + built — counter now `4 of 10` |
| S06 | `Setup-Route-Mismatch.dc.html` | The route resolved to a device other than the selection. **Halt.** | F1, FR-CAP-3, FR-CAP-3a | `Choose another input` → S04. No "continue anyway" | drawn + built — counter now `4 of 10` |
| S07 | `Setup-Level.dc.html` | Set input level against noise, live meter, headroom target | FR-CAP-3, F3 | Level slider (input gain, 0–12 dB in 3 dB steps). `Test` arms the meter. `Continue` is **never** disabled for validation | drawn + built — counter now `5 of 10`. **Amended 2026-09-22 (R-1168, R-1170):** the slider this row has always specified was never built and the board had silently agreed with the code; it now exists, at `AndroidAudioIo.read()`, with the copy retraction it forces (this screen used to promise "the app only reads it"). `Continue enabled only in range` is **withdrawn**, not unbuilt: it stranded an operator whose band would not go green, on a screen that also passes `onBack = null`. The rule is now keep the button lit, validate on tap, record the unresolved state — S12's Level row carries the amber `Fix`. `onBack = null` here is R-1162/R-1170's own open item, untouched by this change |
| S08 | `Setup-Battery.dc.html` | Battery-optimisation exemption. States plainly that the API lies and liveness is proven by heartbeat | FR-PLT-1, F5 | `Open settings` → OS. `Skip` → S09, diagnostic-only, never blocks | drawn + built — counter now `6 of 10` |
...
| S10 | `Setup-Rig-Usb.dc.html` | USB device attach + permission grant, with the re-attach warning | FR-PLT-2, F16 | `Grant` → OS prompt → S11. `Back` → S09b | drawn — counter now `7 of 10`; **built as the honest fallback** until P19: FR-RIG was unbuilt, so every S09 rig row landed on the no-rig-support banner with manual frequency entry (R-285, R-344) |
...
| S11 | `Setup-Rig-Verified.dc.html` | Rig identified, transport named, descriptor and verified command set shown | FR-RIG-2, FR-RIG-3, FR-RIG-14 | `Continue` → S12. `Change radio` → S09 | drawn — counter now `7 of 10`, subtitle names the transport; **unreachable until P19** (only the `RigStatus.Stale` halt renders via `rig-lost`, R-125) |
| S11a | `Setup-Models.dc.html` | **The setup MODELS gate (D43, "everything bundled" no longer holds on the `play` variant).** Whatever the detected tier requires that this build did not carry, each with its own real download state — pending, downloading, installed, or failed with a reason and a retry — plus a Wi-Fi-only toggle (default on). **R-1087:** has its own `8 of 10` segment position now, no longer sharing S12/Ready's — the final gate immediately before Analytics consent and Ready, from whichever earlier step preceded it | **D43, FR-AST-10, FR-AST-11, FR-AST-12** | Tap `Download`/`Retry` on a row → starts/restarts that model's fetch. `Wi-Fi only` toggle. `Continue` disabled until every row reads Installed | drawn — built already (P22, `ModelsSetupScreen.kt`); this round adds the artboard, the `setup-models` scenario/`DebugModelsSetupOverride` seam (a real production gap — `ModelCatalog.entries` never reports a non-bundled row on any variant today, see that object's own kdoc) and the S11a tour steps — not yet swept against a capture. **R-1087 renumbering swept in the same change** (register, `9 of 10`/`8 of 10` were `8 of 8`/`8 of 8` before): the segment row now reads `8 of 10`, filled to 8, not the full bar |
| S11b | `Setup-Analytics-Consent.dc.html` | **Analytics consent (P28, D42).** Shown exactly once, right after Models and before Ready — explains tier 1 in the same terms as CF13 (`Settings-Analytics.dc.html`) and offers tiers 2 and 3 as an explicit, unchecked choice. **R-1087:** has its own `9 of 10` segment position now (previously shared MODELS/READY's own `8 of 8`, telling the operator twice they were on the last step); unlike Models it draws **no back chevron** (`AnalyticsConsentScreen.kt` passes `onBack = null` — never a hard gate) | **FR-ANL-9, FR-ANL-10, AC-180**, D42, D48 | `Also share transcripts and callsigns` toggles tier 2. `Also share audio` toggles tier 3. `Continue` → READY, whatever the two toggles are left at (declining both leaves every other function fully working) | drawn — built already (P28, `AnalyticsConsentScreen.kt`); this round adds the artboard and the S11b tour steps (`setup-analytics-consent/S11b-analytics-consent` at 1.0/2.0/2.0-end) so it can now be captured and compared — not yet swept against a capture. **Copy finding, filed for the register rather than fixed here (outside this unit's file ownership):** the built screen never discloses D48's honest "no destination configured" state the way CF13 does one tap away in Settings — an operator can switch on either opt-in toggle here with no way to know that, in this build, nothing is actually sent anywhere; `AnalyticsConsentScreen.kt` takes no `destinationConfigured` input at all. This board draws the corrected copy: a disclosure line beneath the tier-1 paragraph, reusing verbatim the sentence CF13 shows for the same state, so the two screens can never disagree about what "on" means. R-1085 (this disclosure) was already fixed and closed on an earlier capture; **R-1086 fixed in this round** (the tier 1 paragraph no longer promises ANR collection this build cannot do — no watchdog exists, and `AnalyticsTier1Payload.Crash.isAnr` is deliberately nullable, "never measured"), alongside **R-1087's renumbering** (this board's own segment position, above) |
