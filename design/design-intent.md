...
**Boards that lost their indicator rather than their place:** `Setup-Battery.dc.html` and the whole
rig branch (`Setup-Rig*.dc.html`). Neither is a numbered stage of a first run any more — the battery
prompt is a post-capture fact (AC-189 as amended, R-1161) and the rig branch is entered only when a
rig module with a real CAT implementation exists, which none does today. Both are still in the code,
still have boards, and are still reachable by the tour through `EXTRA_STEP`.
...
| S08 | `Setup-Battery.dc.html` | Battery-optimisation exemption. States plainly that the API lies and liveness is proven by heartbeat | FR-PLT-1, F5, **AC-189 (amended)** | `Open settings` → OS. `Skip` → records the answer. `Back to the app` once setup is complete | drawn + built — **no longer a setup stage at all (P39, D58): no counter, no segment row.** Overnight survival is evidence only a completed capture can produce, so a pre-capture gate on it is unsatisfiable by construction and deadlocked every first-run operator (R-1161). Its new home is a *Keep capture running* prompt on the first missed heartbeat — a capture-status surface, **not built yet**; until it is, this screen is reached only by `EXTRA_STEP` and by Ready's own overnight row |
