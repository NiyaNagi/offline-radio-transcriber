...
| C10 | `Transport-Bar.dc.html` | **The bottom bar, one component with two modes.** Live (level, partial, the highest-priority state as its label) and playback (pause/play, position, scrubber, what is playing), with a live dot kept when playing during capture so capture is never out of sight. Hidden on the screen it expands to. Replaces the live bar. Taps: live → Capture (N08); playing → that transmission; × → back to live, or hidden | IA-1, IA-2, FR-UI-5, constitution IV | **drawn 2026-09-12** — not yet built. **Reverses R-1006's stop-on-leave**: once the bar owns playback, leaving the transmission screen must not stop the audio — the bar is the stop control. The label varies by design, so no test or tour step may match on its text (R-1021) |
...
**Four screens since 2026-09-23 (P39, D58, AC-198..204).** A brand-new operator used to walk
**twelve** and could reach twenty; five of those steps had no skip predicate at all and two disabled
`Continue` with no back and no later. The operator's own report after running it on the device was
*"the setup feels to complicated as well, simplify it to a smaller number of pages"*. The sequence is
now **Welcome → Mode → Listen → Ready**, with **Models** inserted only when something the detected
tier needs is genuinely absent — so the indicator's denominator is **3, or 4 with a download**, both
knowable at launch, which retires R-1087's fixed total of 10 rather than inheriting it.

**Boards deleted in that change, deliberately and named here rather than left to rot:**
`Setup-Mic.dc.html` (the microphone explainer — the system dialog is fired straight from the mode
tap, with its rationale on that surface, AC-204), `Setup-Notify.dc.html` (asked at first capture
start), `Setup-Jurisdiction.dc.html` and `Setup-Analytics-Consent.dc.html` (folded onto Welcome as
acknowledged lines with their full content one tap away — AC-166 and AC-180 were *amended*, not
dropped), and `Setup-Input.dc.html`/`Setup-Verify.dc.html`/`Setup-Level.dc.html`, which merged into
the new `Setup-Listen.dc.html`.

**`Setup-Battery.dc.html` (S08) is deleted too, one row later than the rest — R-1188.** P39 kept the
board, `SetupStep.OVERNIGHT` and `OvernightScreen` on the reasoning that the *Keep capture running*
prompt owed to the first missed heartbeat would land on that screen. It landed on **F24**
(`Fail-Keep-Running.dc.html`) instead, built by the other half of the same wave with its own copy and
its own battery-setting intent — leaving S08 unreachable from any production path and kept alive only
by tests that opened it deliberately. One ask, one surface: two surfaces is how the two frequency
fields in R-1182 drifted apart. The one thing S08 said and F24 did not — *choose Don't optimise or
Unrestricted* on the platform screen — moved into F24's body in the same change. Ready's own Overnight
row survives and still names the fact, but no longer offers a `Fix`: nothing it could route to would
fix it, since the only thing that clears it is a capture that has not happened yet.

**Boards that lost their indicator rather than their place:** the rig branch
(`Setup-Rig*.dc.html`) — not a numbered stage of a first run, entered only when a rig module with a
real CAT implementation exists, which none does today. Still in the code, still with boards, and
still reachable by the tour through `EXTRA_STEP`.

| # | Artboard | Purpose | Serves | Interactions | Status |
|---|---|---|---|---|---|
| S00 | `Setup-Mode.dc.html` | **How is the radio connected?** Three modes, one plain sub-line each — where the sound comes from, in words a scanner listener with no licence can answer. **Stage 1 of 3** (4 with a download). **This is the surface the microphone is asked from** (AC-204): a footer line carries the rationale in the *needs X so you can Y* shape and names what happens next, and the tap fires Android's own dialog directly — there is no explainer step in front of it any more | **FR-CAP-8, FR-CAP-9, AC-204**, D33, D58 | Tap a mode → the system microphone dialog → S04. No `Continue`: a tap is the answer | drawn + built — **redrawn 2026-09-23 (P39)**: the counter, the three sub-lines and the two paragraphs of preset theory beneath them. Not yet swept against a capture at the new copy |
| S01 | `Setup-Welcome.dc.html` | What this app does, the FR-ANL-14 sentence **once and only here** (AC-203), and the two notices this flow used to spend a screen each on — the jurisdiction notice and the analytics disclosure — as acknowledged rows with their full content one tap away. `Begin` is the acknowledgement, and the screen says so | FR-PLT-1, **AC-166, AC-180, AC-203, FR-ANL-14** | `Begin` → S00. Either notice row → its own sheet (the analytics sheet carries the two **unchecked** tier toggles). `What is captured?` → the privacy sheet | drawn + built — **redrawn 2026-09-23 (P39)**: the five-ask list is gone (four of the five asks no longer exist), the two notice rows and the acknowledgement line are new. Not yet swept against a capture |
| S02b | `Setup-Mic-Denied.dc.html` | Microphone refused twice — the OS prompt is gone. Halting, with the settings path | FR-CAP-1, F1 | `Open app settings` → OS. `Check again` re-reads the grant; auto-advances when granted | drawn + built — counter now `1 of 3`, halting that stage: the ask belongs to the mode surface now, so the halt does too |
| S02c | `Setup-Bluetooth-Permission.dc.html` | **Nearby devices** — the `BLUETOOTH_CONNECT` rationale, Bluetooth mode only. Says what it is used for and what denying does | FR-CAP-8, FR-RIG-14, FR-PLT-1 | `Allow nearby devices` → OS prompt → S04. `Not now — use USB instead` → mode becomes USB, S04 | drawn + built — counter now `1 of 3`. **R-1179: every committed capture of this board is the Input screen** — `install.ps1` grants the permission, so the gate could never fire. `DebugBluetoothPermissionOverride` (P39) is the fix, and `ScreenshotTourActivity.assertStillOnRequestedStep` is the guard that would have caught it. Its evidence is void until a run under the new seam |
| S04 | `Setup-Listen.dc.html` | **Find the radio's audio — the one screen that carries the whole audio decision (P39, D58).** The route list with its preset chip, the four-stage route check expanding in place beneath it, then the live meter and its gain. Merges what were S04, S05 and S07: one task, one failure mode, and **the only *silent* one in the flow** — which is why D58 makes this the one screen that must be full-screen | FR-CAP-2, **FR-CAP-2b, FR-CAP-2a, FR-CAP-3, FR-CAP-9, FR-CAP-10, FR-CAP-11, AC-198, AC-201**, constitution IV | Tap a route → selects, and clears any verification of the route it replaces. `Verify this input` runs the check in place. `Continue` once it passes; lit and refusing before that. A mismatch → S06. A timeout → `Try again` / `Choose another input`. `Refresh` re-enumerates | **drawn 2026-09-23 (P39)** — new board, replacing three. Not yet swept against a capture |
| S06 | `Setup-Route-Mismatch.dc.html` | The route resolved to a device other than the selection. **Halt.** | F1, FR-CAP-3, FR-CAP-3a | `Choose another input` → S04. No "continue anyway" | drawn + built — counter now `2 of 3`. Merging Verify into S04 changed where the check is *shown* and nothing about whether it blocks (constitution IV, AC-201) |
| S09 | `Setup-Rig.dc.html` | The rig **catalogue**, generated from the installed descriptor set | FR-RIG-1, **FR-RIG-16, FR-RIG-17, FR-RIG-18, FR-RIG-19** | Tap a rig → S09b. `No radio` → S10 (frequency entry). `Import it` → file picker → S09. `Not now` → S12 | drawn + built — **no counter (P39): not a numbered stage of a first run.** The whole branch is entered only when the mode implies a rig *and* a rig module with a real CAT implementation exists; none does today, so on current builds it does not appear at all. The tour reaches it through the scripted `RigLinkPort` its scenarios install |
| S09b | `Setup-Rig-Transport.dc.html` | **How is the rig linked?** Both transports for the chosen rig, the mode's preset selected, the cost of each stated | **FR-RIG-13, FR-RIG-14, FR-RIG-17**, FR-CAP-9 | Pick a transport. `Connect over …` → S10 (USB) or S10b (Bluetooth). `Back` → S09 | drawn + built — **no counter (P39)**, see S09. The ghost "Back" at the top of the frame is the emulator renderer (R-884/R-946, hardware H14) |
| S10 | `Setup-Rig-Usb.dc.html` | USB device attach + permission grant, with the re-attach warning, and the manual-frequency fallback | FR-PLT-2, F16 | `Grant` → OS prompt → S11. `Back` → S09b | drawn — **no counter (P39)**, see S09. **AC-202: first-run setup never asks for the manual frequency any more** — the value is edited in the log's own header (R-1167); the field and everything downstream of it stay, only the prompt moved. R-1178's branch-specific message is unresolved and unreachable on current builds |
| S10b | `Setup-Rig-Bluetooth.dc.html` | **Bluetooth rig**: the paired-device list, then the open-link → identify → verify checklist | **FR-RIG-14, FR-RIG-15**, FR-RIG-11 | Pick a device. `Continue` once verified → S11. `Continue without connecting` → S10. `Use USB instead` → S10. `Refresh` re-reads the paired list | drawn + built — **no counter (P39)**, see S09; everything else unchanged, including the navigation-bar inset band per guide §10.1 and R-1005c's escape. The live link itself is hardware H2 |
| S11 | `Setup-Rig-Verified.dc.html` | Rig identified, transport named, descriptor and verified command set shown | FR-RIG-2, FR-RIG-3, FR-RIG-14 | `Continue` → S12. `Change radio` → S09 | drawn — **no counter (P39)**, see S09 |
| S11a | `Setup-Models.dc.html` | **The setup MODELS gate (D43).** Whatever the detected tier requires that this build did not carry, each with its own real download state, plus a Wi-Fi-only toggle (default on). **The one conditional stage in the flow**, and the whole reason the denominator is 4 rather than 3 — whether it will be walked is known at launch from the installed-asset state alone | **D43, FR-AST-10, FR-AST-11, FR-AST-12** | `Download`/`Retry` per row. `Wi-Fi only` toggle. `Continue` lit throughout, refusing with what is missing (AC-201) | drawn + built — counter now `3 of 4`. Not yet swept against a capture |
| S12 | `Setup-Done.dc.html` | Summary of what was configured, one action to start. Carries the unresolved items as amber rows — a level that never went green, a model not installed — each with a way back | P8, FR-CAP-12, FR-AST-3, **AC-200** | `Start capture` → the notification permission is asked here, in context, at the first capture start → N01. `Change` on Mode → S00. `Fix` rows as before. `Back to the app` once setup is complete | drawn + built — counter now `3 of 3` (or `4 of 4`). **No longer gated behind the `setupComplete` latch (P39)**: the terminal state is a function of "no gates remain", so re-entry has somewhere to land — which it did not, and that was half of R-1161 |
...
| N07 | `Live-Monitor.dc.html` | **The live instrument.** What the live bar expands into: elapsed, the running level envelope with its target band, the Pass A partial as it arrives, and the overs of this session newest-first, each carrying the state it is actually in — transcribing, waiting behind N others, not transcribed, or done with its attribution marker. Gaps are rows, not absences (FR-RUN-12). Drawn 2026-09-12 for R-1007: the operator ran 30 minutes of capture and could not tell what was happening. `onOpenLive` at `CaptureStatusContent.kt:141` is the existing unwired seam it lands on | FR-UI-7, FR-UI-12, FR-RUN-12, P4, **constitution I** | `Back` → N04. Over row → D01–D04. `Full log` → L01. `Stop` (confirm) | **drawn + built** — WPL `59894e75`. **Five accepted deviations, recorded here so they are not re-raised forever (constitution VIII):** (1) elapsed reuses `CaptureStatusViewState.sinceElapsedLabel` as one sub-line rather than the board's separate large figure, so Capture Status and Live Monitor can never drift apart on the same number; (2) the board's per-partial elapsed-seconds figure is **omitted — no signal for "how long this partial has been streaming" exists anywhere in `:pipeline`**, and inventing one would be fabrication; (3) resolved rows use the existing `AttributionRow` (marker + callsign leading) rather than the board's inline-underlined transcript with a trailing marker, because no lexicon word-position data exists to place the underline and a second parallel attribution rendering would be untested and less accessible than the one component that already is; (4) an AMBIGUOUS row shows no kept-candidate "or QRF" pair — that needs the full candidate inspection this screen does not read — though the marker state itself is correct; (5) the board's "~40 s behind" ETA is **omitted — no honest per-pass timing signal exists**; the queue-position count ("N ahead of it") is real and is shown instead. Deviations 2, 4 and 5 are Principle I applied correctly: the board drew a fact the system cannot yet know. Needs a tour step — `overnight-live-monitor`, routed to the package owning `app/src/debug/**` |
| N08 | `Capture.dc.html` | **Capture as one instrument.** State and elapsed, the level envelope, the Pass A partial, the four status facts (input, radio, queue, storage), and this session's overs in flight, on one scrolling surface. **Supersedes N04, N06 and N07 as separate screens** — those rows and boards stay, marked superseded, nothing deleted quietly (P9). The transport bar is **hidden here while live**, because this screen is what it expands to (IA-2, C10) | FR-UI-7, FR-UI-12, FR-RUN-12, FR-CAP-13, D39 | Status row → its settings. Over → D01–D04. `all in Recordings` → RC02 for this session. `Stop` (confirm) | **drawn 2026-09-12; built, and `ReaderDestination.CAPTURE` has rendered it since `3b5d0e0a` (WPCAP)** — it supersedes N04, N06 and N07 as separate screens, which is why `LiveMonitorScreen` is no longer reachable from any navigation path and the `overnight-live-monitor` tour steps that chased it were removed (R-1021). This row read "not yet built" until 2026-09-21; the lead corrected it after a builder traced three failing tour steps back to it. The board carries this session's findings forward rather than re-creating them: no fixed timestamp width (R-1023), grey/green/red level mapping (R-1026), the status on its own line under the caption (R-1027), and a manual frequency shown as a fact now that R-1030 makes it one |
...
| L06 | `Log-Frequency-Edit.dc.html` | **The hand-entered frequency, editable in the Log's own header** (R-1167, D58, **AC-202**) — where it landed when D58 took it out of first-run setup. `not set` is an honest empty state; a value carries `by hand`; the row is **absent entirely** when a rig is reporting a frequency and nothing overrides it (there is nothing for the header to add), and when a hand-entered value *is* overriding a reporting radio it names both numbers and offers only to stop overriding, never a silent re-entry (FR-RIG-8 gives the manual value precedence). L01's own header carries the collapsed row | **AC-202**, AC-131, FR-RIG-1, FR-CAP-8 | `Set the frequency`/`Change` → the editor expands in place. `Save` is lit at all times and refuses an unparseable entry on the tap (R-1170). `Clear the frequency` is its own labelled action, never a blank field. The editor states when a save takes effect — AC-131 freezes configuration at session start | drawn + built |
...
| ST04 | `Station-Identity.dc.html` | How this station is known: heard callsign, voiceprint cluster, operator-named. Each of the three carries its own rule about leaving the device (R-1173) | FR-SPK-10, constitution III | `Rename`. `Split cluster` (F10) | drawn + built — **accepted deviation**: "stable since <date>" is the first-seen date of the currently bound voiceprint cluster — a real fact — not a pipeline stability judgement, which does not exist; it is omitted when no voiceprint is bound (R-572). **Second accepted deviation (this task, constitution I/VIII, D45)**: the whole Voiceprint row ("One cluster, N overs") draws the state once `:identity` has bound a real voiceprint — `:identity` is an empty stub today and nothing else creates a station's first voiceprint, so every real device reads "No voice on file yet" with a small not-computed marker instead |
...
| RC01 | `Recordings.dc.html` | **Recordings** — every session newest first, with its span, overs, stations, gaps and failures, and whether its raw archive is kept or was removed and on what date. Both storage budgets sit at the top as the policies they are: over audio **warns and is never deleted without the operator** (R-1037), and the raw archive **states that it is on, what it costs per month, and offers Turn off beside the statement** (D39, R-1036). **Replaces Earlier nights (DG03) as the home for sessions** (IA-1); DG03's row stays, superseded | FR-STO-3, FR-STO-3d, D39, P9, FR-OBS-4 | Session → RC02. Chips filter. Storage card → CF03. `Turn off` → archive setting | **drawn 2026-09-12** — not yet built |
| RC02 | `Recording-Session.dc.html` | **One recording.** The session's coverage — overs as ticks (failed in amber), not-listening as hatch, the playhead — then every over in order with its own play control, attribution marker, station link, training label and failure state; a gap is a row with no play control, because nothing was recorded (FR-RUN-12). Session actions: play all, export, label, and **delete with the space it frees shown before it is tapped** | FR-OBS-4, FR-UI-5, FR-RUN-12, FR-STO-3, P9, IA-6 | Row play → transport bar. Callsign → ST02. Over → D01–D04. `Retry` → requeue. `Delete` → confirm sheet repeating the space | **drawn 2026-09-12** — not yet built. `FR-OBS-4` labelling is specified since draft 1 and has never been built |
...
| CF13 | `Settings-Analytics.dc.html` | **Analytics (P28, D42).** Three toggles, one per tier — tier 1 (usage and quality) on by default and can be turned off; tier 2 (transcripts and callsigns) and tier 3 (audio) opt-in and off — each stating in plain language what that tier sends (FR-ANL-2..4's own field lists verbatim); a subtitle carrying D48's honest state (queued-and-unsent when no destination is configured, sends-only-outside-capture once one exists); a "never in any tier" statement; this install's id with a reset action | **FR-ANL-1, FR-ANL-2, FR-ANL-3, FR-ANL-4, FR-ANL-9, FR-ANL-7, FR-ANL-11**, D42, D48 | Tier 1 toggle can turn tier 1 off entirely. Tier 2/tier 3 toggles opt in, start unchecked. `Reset install id` → starts a new, unlinked id | drawn — built already (P28, `SettingsAnalyticsScreen.kt`); this round adds the artboard and its own design-intent row (tour ids `overnight/CF13-settings-analytics` at 1.0/2.0/2.0-end) so it can now be captured and compared — not yet swept against a capture. **No copy finding on this screen**: the built copy already states each tier's field list plainly and already carries the no-destination-configured disclosure this board draws, via `SettingsAnalyticsViewState.destinationConfigured` — contrast S11b below, which has no equivalent |
| CF14 | `Settings-Backup.dc.html` | Full database-and-audio backup and restore — a device-to-device transfer, real counts before every write, restore conflicts shown never auto-resolved | **FR-STO-6, FR-STO-9, AC-170** | `Save backup`. `Choose a backup file` → plan → `Restore`/`Cancel` | drawn + built (P30). Analytics (P28, `SettingsAnalyticsScreen`, tour id CF13) has no artboard yet — a pre-existing gap this unit did not introduce and is out of this unit's own ownership to fix |
| CF15 | `Settings-Alerts.dc.html` | Live alerts — a master on/off switch, an honest amber banner when the platform's notification permission is denied, and add/edit/remove for a callsign, a keyword and a frequency watch, each with its own enable toggle | **FR-ALR-1, FR-ALR-2, FR-ALR-5, FR-ALR-6** | `Alerts` toggle. Per-watch toggle, `Edit`, `Remove`. `Add a callsign`/`Add a keyword`/`Add a frequency, MHz` | drawn + built (P31, tour ids `overnight/CF15-settings-alerts` at 1.0/2.0/2.0-end) — not yet swept against a capture |
...
| F24 | `Fail-Keep-Running.dc.html` | **The battery-exemption ask, raised by the first missed heartbeat, and the only battery surface in the product** (R-1161, R-1162, R-1188, D58, **AC-189 as amended**). The old `SetupStep.OVERNIGHT` refused to start capture until overnight survival was proven, and only a capture can prove it — a deadlock with no exit but uninstalling; R-1188 deleted that screen and its board outright, which also resolves **R-1162** (its finding was that the screen had no exit; this is a dismissable banner over a working app). Overnight survival is a post-capture fact, so this is where it lives: amber (capture is running), a banner and never a takeover, triggered by a real gap in the app's own heartbeat trail and never by `isIgnoringBatteryOptimizations()`, which lies on the reference device. The caveat is load-bearing: the copy says plainly that the setting is not a guarantee, and that whatever the operator chooses it never blocks capture (**AC-199**). **R-1188 added the one instruction the deleted screen carried and this did not** — which option to pick on the platform screen that opens | **AC-189**, AC-199, FR-SVC-5b, NFR-8, constitution IV | `Open the battery setting` → the platform intent. `Not now` answers *this* gap and persists; a strictly newer missed heartbeat is new evidence and asks again, the same gap never twice | drawn + built — tour (`keep-running/F24-keep-running-now` at 1.0/2.0/2.0-end) |
